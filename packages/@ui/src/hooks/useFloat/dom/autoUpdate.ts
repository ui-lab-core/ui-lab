    ...
    function handleObserve(entries: IntersectionObserverEntry[]) {
      ...
      if (ratio !== threshold) {
        ...
        if (!ratio) {
          ...
          // to prevent an infinite loop of updates. This 1000ms delay only applies
          // to edge cases (completely clipped elements), not normal scroll updates.
...
export function autoUpdate(
  ...
  // Attach scroll listeners directly to ancestors without debouncing.
  // Each scroll event triggers an immediate position update via update(),
  // ensuring the floating element follows scroll movement in real-time.
  // passive: true improves scroll performance without affecting update timing.
