...
const THEME_CACHE_KEY = 'uilab_theme_complete';
...
function getDefaultSourceConfig(mode: "light" | "dark"): ThemeSourceConfig {
  return {
    colors: { background: { h: 0, c: 0, l: 0.15 }, foreground: { h: 0, c: 0, l: 0.98 }, accent: { h: 210, c: 0.15, l: 0.5 } },
...
export function cacheCompleteTheme(cssVariables: Record<string, string>, sourceConfig: ThemeSourceConfig): void {
  if (typeof window === 'undefined') return;
  try {
    const toCache: CompleteThemeCache = { cssVariables, themeMode: sourceConfig.mode, sourceConfig, timestamp: Date.now(), version: 1 };
...
export function clearCompleteThemeCache(): void {
  if (typeof window === 'undefined') return;
  try { localStorage.removeItem(THEME_CACHE_KEY); } catch {}
}
