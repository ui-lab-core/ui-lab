...
```
F12 → Elements → Look for <link> to ui-lab-components/styles.css
```
...
```css
/* ❌ Too generic – can be overridden */
button {
  background: blue;
}

/* ✅ Use CSS Modules for proper scoping */
.button {
...
```tsx
import { Button } from 'ui-lab-components';
// No styles imported – button is unstyled!
```
...
useEffect(() => {
  ...
  // Missing cleanup – timer runs forever
}, []);
```
...
3. **Keep CSS variables** – Don't inline styles
4. **Code split by route** – Reduce initial load
5. **Monitor bundle size** – Use webpack-bundle-analyzer
6. **Use CSS modules** – Scoped styles are more efficient
7. **Tree-shake unused code** – Only import what you use
