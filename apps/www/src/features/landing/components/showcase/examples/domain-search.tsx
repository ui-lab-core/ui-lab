...
const TLDS = [
  ...
  { tld: ".so", price: 24, taken: false, popular: false },
];
...
export function DomainSearch() {
  ...
  return (
    <div className="h-fit w-full bg-background-900 border border-background-700 rounded-sm overflow-hidden">
      ...
      <List items={TLDS} spacing="default" className="max-w-none">
        {TLDS.map((item, i) => {
              ...
              <List.Item value={item.tld} className="px-4 py-3">
                <List.Media>
                  <div className="w-8 h-8 rounded-sm bg-background-800 flex items-center justify-center shrink-0">
                    <Globe size={15} className={item.taken ? "text-foreground-400" : "text-foreground-300"} />
                  </div>
                </List.Media>
