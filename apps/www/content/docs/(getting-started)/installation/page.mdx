...
**Fastest: Start from a template.** Browse pre-built [Starters](/workshop/starters)—Next.js, Vite, Astro, Tauri, and more. Clone, install, build. Everything's configured.
...
Each component automatically loads its own default styles when you import it. Pair the components with a theme package to provide the design tokens those defaults consume.
...
That is the only required UI Lab stylesheet import:

1. `tailwindcss` establishes the layers.
2. `ui-lab-theme-onyx/styles.css` defines the design tokens (colors, typography, spacing).
3. Imported components bring in only their own default CSS. The package's cascade-layer contract keeps theme overrides above those defaults regardless of module evaluation order.

`ui-lab-components/styles.css` remains available as an optional aggregate bundle for environments that deliberately load the entire component library at once.

## Add long-form typography

UI Lab keeps typography intentionally role-based: body copy inherits `--font-body`, all headers use `--font-header`, and code uses `--font-mono`. Import the optional typography layer after the component styles, then add `typography` to the article or rich-text container—not to the page root.
...
<article className="typography">
  <h1>Release notes</h1>
  <p>Paragraphs, headings, lists, tables, links, media, and code now share one reading rhythm.</p>
</article>
```

The layer is scoped, so component UI remains untouched. It establishes document flow first, then gives headers their dedicated type role. To customize it, override the role tokens in your app theme:

```css
@theme {
  --font-body: "Inter", ui-sans-serif, system-ui, sans-serif;
  --font-header: "Newsreader", ui-serif, Georgia, serif;
...
See [ui-lab-theme-onyx](https://github.com/ui-lab-core/ui-lab/tree/main/packages/@theme-onyx) for the complete token structure.
...
- Explore [Elements](/workshop/elements) for composed UI patterns
- Check [Starters](/workshop/starters) for complete project templates
