...
Import this stylesheet exactly once, before any other app or component module, from your application entry point (for example the root layout import in Next.js). `ui-lab-components` registers its component CSS in cascade layers; if a module that transitively imports it runs before your global stylesheet, those layers register before Tailwind's, and Tailwind's base styles can override UI Lab's component styles.
